...
4. **Service** — the actual business logic, named and structured per django-services. Use Loguru for service logging — `logger.info("event name", key=value, ...)` with structured key-value pairs, never interpolated strings. View `except` blocks are the exception: they use the f-string + traceback form from django-errors.
5. **API + serializers + URL** — plain ViewSet per resource, per-action try/except in every action, per-action Input/Output serializers in `serializers/<entity>.py`, Router-registered URLs (django-apis).
6. **Exceptions** — every view action gets the standard try/except ladder (django-errors); add `ApplicationError` subclasses in the app's `exceptions.py` only if the feature has business-rule errors.
